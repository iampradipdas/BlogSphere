        ...
        <div class="p-6 mt-2 text-gray-900 bg-gray-50">
            <form action="{{ route('roles.update', $role->id) }}" method="post">
                ...
                <div>
                    ...
                    <div class="my-3">
                        <input value="{{ old('name', $role->name) }}" name="name" placeholder="Enter Name" type="text"
                        ...
                        @foreach ($permissions as $permission)
                            <div class="flex items-center space-x-2">
                                <input type="checkbox" name="permissions[]" value="{{ $permission->name }}"
                                    id="perm_{{ $permission->id }}" class="rounded"
                    ...
                    <button class="bg-slate-700 text-sm rounded-md text-white px-3 py-2 mt-4">Update</button>
                </div>
            </form>
        </div>
    </div>
